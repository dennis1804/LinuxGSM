		...
		if [ "${counter}" -le "10" ]; then
			...
			if [ "${counter}" -le "4" ]; then
				if [ "${appid}" == "90" ]; then
					...
					local exitcode=$?
				elif [ "${appid}" == "302550" ]; then
					${unbuffer} ./steamcmd.sh +@sSteamCmdForcePlatformType windows +login "${steamuser}" "${steampass}" +force_install_dir "${serverfiles}" +app_update "${appid}" ${branch} validate +quit
					local exitcode=$?
